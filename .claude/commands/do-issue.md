...
// TRIVIAL: Tests that component renders without testing behavior
test('renders without crashing', () => {
  render(<Component />);
});

// TRIVIAL: Snapshot tests without specific purpose
...
// MVP-LEVEL: Tests core functionality directly related to acceptance criteria
test('validates input according to business rules', () => {
  render(<Component />);
  
  const input = screen.getByRole('textbox');
  fireEvent.change(input, { target: { value: 'invalid-data' } });
  
  // Test the actual validation behavior, not the UI details
  expect(screen.getByText(/invalid format/i)).toBeInTheDocument();
});
```

**MVP Test Checklist:**
- [ ] Each test directly verifies an acceptance criterion
- [ ] Tests focus on user-facing functionality
- [ ] No duplicate or redundant test coverage
- [ ] Tests verify behavior, not implementation
- [ ] Edge cases limited to those in acceptance criteria
...
Before manual testing, I'll run automated verification using BrowserMCP to identify and fix issues early:
...
### 🤖 BrowserMCP Automated Testing Results:

I'll review the automated verification report and address any critical issues found:

- [ ] **Critical Issues**: Fix blocking problems immediately
...
**BrowserMCP Report**: [Link to generated report in `.claude/verification-reports/`]

### Iteration Loop for Issues Found:

If BrowserMCP finds issues, I'll:
1. **Implement fixes** based on specific findings
2. **Re-run verification** to confirm fixes
3. **Iterate** until critical issues are resolved
4. **Proceed** to manual testing with reduced scope
...
With automated verification complete, this is a **focused manual verification** point to review aspects not covered by automation.
...
Update Todos
  ...
  ☒ Build Phase
  ☒ Test Fixes Phase
  ☐ Manual Testing ← WE ARE HERE (FOCUSED REVIEW AFTER AUTOMATION)
  ☐ Cleanup & Documentation Phase
  ☐ GitHub Issue Management
```
...
- ✅ **Storybook Testing**: BrowserMCP verified all stories, controls, and visual states
- ✅ **Test Harness Testing**: Automated navigation and interaction testing
- ✅ **Responsive Design**: Multi-viewport testing completed
- ✅ **Accessibility**: Automated compliance checks completed
- ✅ **Performance**: Load time and responsiveness verified
- ✅ **Integration**: Cross-component and context testing completed

### 👁️ Focused Manual Verification (Reduced Scope):

#### 1. BrowserMCP Report Review:
- [ ] Review the automated verification report for any issues
- [ ] Check if any critical issues were found and resolved
- [ ] Verify BrowserMCP covered all expected scenarios

#### 2. Business Logic Validation:
- [ ] Verify feature aligns with business requirements
- [ ] Test any complex domain-specific scenarios
- [ ] Confirm edge cases specific to the business logic

#### 3. Strategic User Experience Review:
- [ ] Spot-check critical user flows manually
- [ ] Verify feature provides expected user value  
- [ ] Test any nuanced interactions not covered by automation
...
```bash
# View BrowserMCP verification report (primary)
cat .claude/verification-reports/issue-$ARGUMENTS-*.md

# Re-run BrowserMCP if needed
./scripts/browsermcp-verify.sh $ARGUMENTS full

# Targeted re-testing if issues found
...
# Manual verification (fallback if needed)
npm run storybook  # Story review
npm run dev        # Manual testing
git diff HEAD~1 HEAD  # Code review
```
...
I'll now clean up and document the implemented feature, including removing any trivial tests or stories:
