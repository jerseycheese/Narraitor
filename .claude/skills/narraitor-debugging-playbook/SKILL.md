...
| Theming drift (one DS wrong) | Reproduce in Storybook toolbar across ds1/ds2/ds3 + dark; inspect computed style of the token (`getComputedStyle(el).getPropertyValue('--color-…')`) | Token defined in one theme file only; legacy `--primary` HSL tokens vs `--color-*` drift (#1474); selector specificity vs `.app-surface-*` heading rules | Hardcoding the "right" color (stylelint will block it — and it's wrong) |
