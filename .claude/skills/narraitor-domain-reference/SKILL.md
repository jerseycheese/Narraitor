...
- Tone settings flow into both prompts and per-request safety settings (`parseContentRating` reads the rating out of the prompt, and the Gemini adapter's `getSafetySettingsForRating` turns it into thresholds); content rating is a player-facing feature, not boilerplate.
