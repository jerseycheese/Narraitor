...
## E18. Making the alignment tag order unpredictable (#1829, closed unfixed 2026-08-24)

- **Symptom:** Every turn offered the same three option shapes in the same order, so a player could read slot 3 as "the reckless one" without reading it. Baseline was `lawful/neutral/chaotic` on 29 of 30 turns, and the blind judge in #1818 described the fixed shapes independently.
- **Wrong path:** Two attempts, both aimed at the prompt's own listing order. #1877 reordered the alignment glossary statically; round 6 (PR #1929) rotated it through four permitted orders per turn, keyed off an uncapped `turnIndex`. Both were measured on live 30-turn runs. Dominant ordering went 96.7% at baseline, 70.0% after #1877, then back up to 83.3% after the rotation, against a bar of roughly 60%. Slot-predicts-tag was worse on every slot in round 6 than in round 5.
- **Root cause of the dead end:** the model does not read the glossary's listing order as an instruction about output order. Turn 1's glossary listed NEUTRAL, CHAOTIC, LAWFUL and turn 20's listed LAWFUL, CHAOTIC, NEUTRAL, and both turns produced options in NEUTRAL, LAWFUL, CHAOTIC order. The `debugInfo` wiring added in the same PR showed the model emitting an `Alignment Mix:` line stating its intended order on 30 of 30 turns and then writing the options in a different order than the one it had just stated. Few-shot ordering is ruled out too: `CHOICE_EXAMPLES` is imported only by `playerChoiceTemplate`, and the live path is `alignedChoiceTemplate`. The dominant output order is in fact one of the two orders `ALIGNMENT_GLOSSARY_ORDERS` is forbidden to display.
- **Evidence:** known. Eval logs `1829-round6-verification.md` and `1829-alignment-and-continuity-round5.md` under `narraitor-prompt-template-governance/eval-logs/`; raw captures under `~/.claude/projects/-Users-jackhaas-Projects-personal-narraitor/artifacts/1829-round6-verify-*`; the closing comment on #1829 and the build-order comment on #1834.
- **Doctrine:**
  (a) **Do not reopen #1829 with another prompt-side ordering change.** Nothing that lists the tags in the prompt predicts what comes back. A reopen needs a post-parse mechanism: permute options in `choiceGenerator` after parsing, or derive tags post-hoc from generated options rather than requesting them.
  (b) **A mechanism verified working is not a fix.** Round 6's rotation was confirmed correct at eight sampled turns past the point where a Codex-caught bug would have frozen it, and the player-facing metric still got worse. Verify the mechanism and the metric separately, and report the metric.
  (c) **One 30-turn run per arm cannot separate a real effect from run-to-run variance.** #1877's 70.0% read as progress from 96.7% and did not hold; the next run on a strictly stronger mechanism came back at 83.3%. Treat the trend across runs as the unit, and treat a single improved run mid-arc as unconfirmed.
- **Encoded in:** failure-archaeology index; ai-quality-discipline section 7a; prompt-template-governance eval logs.
