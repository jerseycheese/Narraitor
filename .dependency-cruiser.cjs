...
module.exports = {
  forbidden: [
    ...
    // Narraitor-specific domain boundary rules
    {
      name: 'state-no-component-imports',
      severity: 'error',
      comment:
        'State stores should not import from components. This violates clean architecture - ' +
        'state should be pure domain logic without UI concerns. Move shared logic to lib/ or utils/.',
      from: {
        path: '^src/state/'
      },
      to: {
    ...
    {
      name: 'state-no-cross-domain',
      severity: 'warn',
      comment:
        'Stores should not directly depend on other stores. Use composition patterns or ' +
        'extract shared logic to lib/. This keeps your state management modular and testable.',
      from: {
        path: '^src/state/([^/]+)Store\\.ts$'
      },
      to: {
        path: '^src/state/(?!\\1)([^/]+)Store\\.ts$',
        pathNot: '^src/state/types\\.ts$'
      }
    },
    {
    ...
    {
      name: 'utils-no-state-imports',
      severity: 'error',
      comment:
        'Utility functions should not depend on state. Utils should be pure, reusable functions ' +
...
// generated: dependency-cruiser@17.3.6 on 2026-01-05T02:43:54.176Z
