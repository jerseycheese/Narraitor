...
- **Named page widths**, the tier below the app shell: `--page-width-narrow` (`64rem`/1024px — hero content, ending screen, landing's page container) and `--page-width-prose` (`48rem`/768px — legal text, about's lead copy and hero visual). Both live in [_shared-tokens.css](src/lib/theme/themes/_shared-tokens.css) alongside `--page-gutter` (`var(--space-4)`), the shared horizontal inset from viewport edge to content.
