...
Play a story in any world you can imagine. Build a setting, create a character, and make the choices that steer what happens. Your decisions get tested against your character's skills, so what comes next is earned rather than handed to you.
...
The part that matters is that it's not just generic fantasy. A noir detective setting reads completely differently from a space opera, because the storytelling adapts to the world you described rather than defaulting to swords and dragons. Middle Earth, the beaches of Normandy, something you invented last week — all fair game.

## What to know before you start

**You bring your own key.** Generation runs on a [Google Gemini](https://aistudio.google.com/apikey) key you provide once under Settings, then Providers. It's encrypted in your browser and sent per request, so your stories run on your account, not someone else's.
...
**Build a world.** Describe a setting and the creation wizard suggests attributes and skills that fit it — "Force Sensitivity" for Star Wars, "Sanity" for Lovecraft — which you can then edit, replace, or write from scratch.
...
**Keep track of it.** A journal drawer holds story history and past decisions, inventory tracks what you're carrying, and "Story So Far" summaries capture where things stand at pivotal moments — which also keeps long campaigns coherent instead of drifting.

**Finish it.** When a story's reaching its natural end, you get an ending suggestion and a generated conclusion rather than a session that just trails off.
...
That's it. The app comes up on `localhost:3000` at the landing page; **Start your story** takes you into world creation.

If you want a server-side key instead of going through the provider settings screen — handy for local development so you're not re-entering it — copy `.env.example` to `.env.local` and set `GEMINI_API_KEY`. That's a fallback for local work only; in normal use the player's own key wins.
...
The approach here is component-first with Storybook and TDD — build components in isolation, then integrate. It keeps things manageable.
...
**World and character creation.** Multi-step wizards that adapt to each other — the world defines the attributes and skills, character creation allocates against them. AI suggestions seed both, and everything stays editable.

**Narrative engine.** Gemini handles generation through Next.js API routes (`/api/narrative/generate`, `/api/narrative/choices`, and others). The interesting part is context management: prompts carry your world's rules, character details, and recent story history so what gets generated stays consistent with the setting rather than drifting into generic prose.
...
**Rate limiting.** The two narrative generation routes (`/api/narrative/generate` and `/api/narrative/choices`) are rate limited — 50 requests per hour per IP in production, looser in development. It's an in-memory limiter, so on serverless it's per-instance rather than a global cap. Requests are sanitized and validated server-side.
...
Two branches matter. `main` is the latest tagged release and the default clone target — pin here if you want something stable. `develop` is the rolling integration line where in-flight work lands, so it may include partial features at any given moment. Contributor PRs should target `develop`.
