...
for i in {1..5}; do npm run test:visual -- tests/visual/flaky-test.spec.ts; done
```
...
npm run test:visual:headed -- tests/visual/flaky-test.spec.ts

# Or debug step by step
npm run test:visual:debug -- tests/visual/flaky-test.spec.ts
```
...
npm run test:visual:update -- tests/visual/specific-test.spec.ts
```
...
git add tests/visual/character-card.spec.ts-snapshots/
