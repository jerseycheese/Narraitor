...
- As a player, I want system events recorded so I have a complete history of my game session
  ...
  **Acceptance Criteria**:
  1. The system automatically creates `JournalEntry` objects for major narrative events, player decisions, discoveries, and character encounters as defined in the MVP scope.
  2. Each entry is assigned the correct `JournalEntryType` ('narrative', 'decision', 'discovery', 'character').
  3. Entries include a `timestamp`, `worldId`, `characterId`, and `sessionId`.
  4. Entry `content` accurately reflects the event or decision.
...
- As a player, I want to browse journal entries chronologically so I can follow the story progression
  ...
  **Acceptance Criteria**:
  1. The Journal UI displays entries sorted chronologically by `timestamp` (newest first).
  2. Navigation allows viewing the details of a selected entry.
...
- As a player, I want to read entries with proper formatting so they're easy to read
  ...
  **Acceptance Criteria**:
  1. The Journal UI can be accessed from the main game session interface.
  2. The `EntryDetail` view displays the full `content` of the selected entry with basic formatting (paragraph breaks).
...
- As a player, I want my journal to persist between sessions so my story history is preserved
  ...
  **Acceptance Criteria**:
  1. Journal entries are saved persistently using IndexedDB via the State Management system.
  2. Entries from previous sessions are loaded correctly when a campaign is resumed.
...
- As a player, I want to see summaries of longer entries so I can quickly understand their content
  ...
  **Acceptance Criteria**:
  1. The AI Service is used to generate a concise `summary` for each created `JournalEntry`.
  2. The generated summary is stored with the `JournalEntry`.
  3. The Journal UI displays the `summary` in the list view (`EntryCard`).
