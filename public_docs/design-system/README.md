...
- **Second theme?** This isn't a simple checklist anymore — [ADR-013](../architecture/ADR-013-collapse-to-single-design-system-ds3.md) collapsed to one design system deliberately, so going back to multiple means re-introducing what that ADR removed, not just adding a file. `readStoredTheme()` in [ThemeProvider.tsx](../../src/lib/theme/ThemeProvider.tsx) is now a stub that always returns `DEFAULT_THEME` — there's no persistence or validation left to update, it'd need to be rebuilt. There's no showcase-page pattern to extend either; ADR-012 retired that in favor of Storybook, whose toolbar currently only switches light/dark (see [.storybook/preview.tsx](../../.storybook/preview.tsx)) — a theme selector would need to be added from scratch. Read ADR-013's rationale before reaching for this.
