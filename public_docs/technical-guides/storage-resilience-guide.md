...
When storage fails, the system automatically falls back to memory storage so the game continues.
...
// Pass onStatusChange if you need to log fallback transitions.
const storage = new ResilientStorageMiddleware({
  onStatusChange: (status, notice) => logger.warn(status, notice?.message),
});
```
...
The Zustand stores already use this through `createIndexedDBStorage()`. No special setup needed.
