...
function createWorkflowTodos(issueNumber) {
  const todos = Object.entries(WORKFLOW_PHASES).map(
    ([phaseId, phase], index) => ({
...
function generateSubagentCommands(phaseId, issueNumber) {
  ...
  const commands = taskSpec.tasks.map((task) => {
    const commandFile = path.join(
      WORKFLOW_DIR,
      `${task.subagentId}-${issueNumber}.md`
    );
...
function main() {
  ...
  if (!command || !issueNumber) {
    console.log(
      'Usage: node workflow-coordinator.js <command> <issue-number> [phase]'
    );
  ...
  switch (command) {
    ...
    case 'delegate':
      ...
      console.log(
        `Subagent commands generated for phase: ${delegationResult.phase}`
      );
