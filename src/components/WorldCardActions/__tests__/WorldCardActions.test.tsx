...
describe('WorldCardActions', () => {
  // Test case for rendering action buttons for active world
  test('renders play, edit, and delete buttons', () => {
    render(<WorldCardActions isActive={true} onPlay={jest.fn()} onEdit={jest.fn()} onDelete={jest.fn()} />);
  ...
  test('calls correct handlers when buttons are clicked', () => {
    ...
    render(<WorldCardActions isActive={true} onPlay={mockOnPlay} onEdit={mockOnEdit} onDelete={mockOnDelete} />);
