...
export default function FinalizeStep({
  ...
  const finalizeFormState = useFormState({
    initialData: {
  ...
  const handleGenerateImage = useCallback(async (customPrompt?: string) => {
    ...
    const image = await imageGenerationState.execute(async () => {
      ...
      return await imageGenerator.generateWorldImage(tempWorld, customPrompt);
    });
    
    if (image) {
      onUpdateWorldData({ image });
    }
  }, [onUpdateWorldData, worldData, imageGenerationState]);
  ...
  useEffect(() => {
    const shouldAutoGenerate = 
      ...
      !finalizeFormState.data.hasAutoGenerated && // Haven't auto-generated yet
      !imageGenerationState.isLoading; // Not already generating

    if (shouldAutoGenerate) {
      console.log('[FinalizeStep] Auto-generating world image for AI-generated world');
      finalizeFormState.updateField('hasAutoGenerated', true);
      handleGenerateImage();
    }
  }, [worldData, onUpdateWorldData, finalizeFormState.data.hasAutoGenerated, imageGenerationState.isLoading, handleGenerateImage, finalizeFormState]);
