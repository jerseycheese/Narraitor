...
const logoSizes: Record<LogoIconSize, number> = {
  small: 32,
  medium: 64,
  large: 96,
  xl: 128,
...
/**
 * The mark's artwork, inlined so it can take `currentColor`.
 *
 * The same drawing also ships as a standalone asset at `public/narraitor-logo.svg`
 * for the favicon and the Open Graph image, which need a file on disk and render
 * against a known background. That copy keeps its fixed black fill. This one has
 * to follow whatever surface it sits on, and a fixed-color raster of it went
 * invisible against the dark header. Regenerate both from the same source if the
 * drawing changes.
 */
function LogoMark() {
  return (
...
export function LogoIcon({ size = 'medium', className = '' }: LogoIconProps) {
  const edge = logoSizes[size];

  return (
    <svg
      className={clsx('logo-icon', className)}
      width={edge}
      height={edge}
      viewBox="150 150 724 724"
      preserveAspectRatio="xMidYMid meet"
      role="img"
      aria-label="Narraitor"
