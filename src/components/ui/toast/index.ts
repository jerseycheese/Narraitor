 ...
 * 
 * @example
 * ```tsx * // 1. Set up the toast system in your app root * function App() { * return ( * <ToastProvider> * <YourAppContent /> * <Toaster position="bottom-right" maxToasts={5} /> * </ToastProvider> * ) * } * * // 2. Use toasts in your components * function MyComponent() { * const toast = useToast() * * const handleSave = async () => { * try { * await saveData() * toast.success('Saved successfully', 'Your changes have been saved.') * } catch (error) { * toast.error('Save failed', 'Please try again later.') * } * } * * return <button onClick={handleSave}>Save</button> * } *```
 * 
 * @integration
