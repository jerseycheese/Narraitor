...
import { GoalType, GoalPriority, NarrativeGoal, GoalExtractionRequest } from '../../types/goal.types';
...
describe('goalExtractor', () => {
  ...
  describe('extractGoalsFromNarrative', () => {
    ...
    test('should handle multiple goals in complex narrative', async () => {
      ...
      expect(goalTitles.some(title => title.includes('artifact'))).toBe(true);
      expect(goalTitles.some(title => title.includes('daughter') || title.includes('rescue'))).toBe(true);
      expect(goalTitles.some(title => title.includes('message') || title.includes('deliver'))).toBe(true);
