...
/**
 * Personalization helpers - aggregate context for the LLM.
 *
...
export function createPersonalizedContext(
  ...
  const analysis = analyzePlayerBehavior(
    character,
    decisions,
    relationships,
    goals
 ...
 */
export function generateNarrativeEnhancement(
  context: PersonalizedNarrativeContext
): string {
  const parts: string[] = [];
 ...
 */
function getDefaultAnalysis(): PersonalizationAnalysis {
  return {
    detectedTraits: [],
    preferences: {
      narrativeStyle: 'exploration',
