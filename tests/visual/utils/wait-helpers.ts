...
export async function expandAllCollapsibleSections(page: Page): Promise<void> {
  try {
    // Find all collapsed section toggle buttons (showing '+')
    ...
    if (count === 0) {
      return; // No collapsed sections to expand
    }
    
    // Click all collapsed sections quickly
    for (let i = 0; i < count; i++) {
      try {
        await collapsedSections.nth(i).click({ timeout: 1000 });
      } catch {
